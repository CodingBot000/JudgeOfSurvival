...
import {
  aliveActors,
  aliveCount as selectAliveCount,
  averageStat as averageActorStat,
  findAliveActor,
  highestFrom,
  lowestFrom,
...
export function createInitialState(metadata = {}) {
  ...
  const { seed, ...stateMetadata } = initialMetadata;
  const state = {
    ...stateMetadata,
...
export function aliveCharacters(state) {
  return aliveActors(state, "characters");
}

export function aliveCount(state) {
  return selectAliveCount(state, "characters");
}
...
function runEventRulesMut(state) {
  ensureCrisisFieldsMut(state);
  recalculateSocialPressureMut(state);
  return runWeightedEventMut(state, EVENT_DEFINITIONS, {
    phaseForTurn: (currentState) => phaseForTurn(currentState.boat.turn),
    isOnCooldown: isEventOnCooldown,
    recentEventCount,
    registerEvent: registerEventMut,
    runFallback: (currentState) => {
      eventSilentTurnMut(currentState);
      registerEventMut(currentState, "silent_turn", null, null, 1);
      return true;
    },
  });
}
...
function findAliveCharacter(state, id) {
  return findAliveActor(state, id, "characters");
}

function averageStat(state, statName) {
  return averageActorStat(state, statName, "characters");
}
